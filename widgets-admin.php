<?php
/*
  Plugin Name: Page Widgets
  Plugin URI: http://www.github.com/voceconnect/page-widgets
  Description: A better interface for managing your widgets.
  Author: johnciacia
  Version: 0.1
  Author URI: http://www.github.com/johnciacia
 */

if( !class_exists( 'Post_Widgets' ) ) {
    /**
     * 
     * @class Post_Widgets 
     */
	class Post_Widgets {
		...
		public static function sidebars_widgets( $sidebars_widgets ) {
			global $post;
			if( $post == NULL ) return $sidebars_widgets;

			$sidebars = get_option( 'page_widgets', array() );
			if( $sidebars == "" ) return $sidebars_widgets;
		...
		public static function admin_enqueue_scripts() {
			...
			wp_enqueue_style( 'widgets-admin', plugins_url( 'css/widgets-admin.css', __FILE__ ) );
			wp_enqueue_script( 'widgets-admin', plugins_url( 'js/widgets-admin.js', __FILE__ ), array( 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable' ), false, true );

			// Get all of the sidebars and widgets. Used by the widgets-order AJAX call
			$sidebars_widgets = get_option( 'sidebars_widgets', array() );
			if( $sidebars_widgets == "" ) return;

			foreach( $sidebars_widgets as $key => &$sidebar ) {

				// WordPress adds an 'array_version' key for internal use.
				if( 'array_version' === $key || count($sidebar) == 0) continue;
		...
		public static function sidebar_admin_metabox() {
			...
			<div id="widget-list" class="column-1">
				<strong><?php _e( 'Avilable Widgets' ); ?></strong>
		...
		public static function get_sidebars() {
			...
			foreach( $wp_registered_sidebars as $sidebar ) {
				...
				if( strpos( $sidebar['id'], self::WIDGET_ID_PREFIX) === 0 ) continue;
				?>
